---
title: "trogoderma granarium on corn or maize"
author: "Dalon White"
date: "2025-08-05"
...
params:
  ...
  # Commodity taxonomy:
  commodity_taxonomic_name: NULL #NULL or ["any taxonomic level to match", "any taxonomic level to match", "etc."]
  commodity_common_name: NULL #NULL or ["corn", "sorghum", "etc."]
...
# commodity_taxonomic_name = params$commodity_taxonomic_name
# common_name = params$commodity_common_name
...
# Generate a detailed report of the search parameters and methodology
generate_search_report <- function() {
  # Create report content with HTML formatting
  report_content <- paste0(
    "<html><head>",
    "<title>Search Parameters Report</title>",
    "<style>",
    ...
    # Report header
    "<h1>Search Parameters and Methodology Report</h1>",
    "<div class='timestamp'>Generated on: ", format(Sys.time(), "%A, %B %d, %Y at %I:%M %p"), "</div>",
    
    # Summary section
    "<div class='summary'>",
    "<p>This report documents a search for pest interceptions in the ARM database with the following parameters.</p>",
    "<p>Found ", nrow(df_diagnostic_results), " matching interception records.</p>",
    "</div>",
    ...
    "<p>The following steps were taken to generate this data:</p>",
    "<ol>",
    "<li>Connected to ARM database and retrieved diagnostic results</li>",
    if(!is.null(params$begin_date) || !is.null(params$end_date)) 
      "<li>Filtered results by date range</li>" else "",
    if(!is.null(params$origin_country)) 
      "<li>Filtered results by origin country</li>" else "",
    if(!is.null(params$commodity_common_name) || !is.null(params$commodity_taxonomic_name)) 
      "<li>Filtered results by commodity names using string pattern matching</li>" else "",
    if(!is.null(params$KINGDOM) || !is.null(params$PHYLUM) || !is.null(params$CLASS) || 
       !is.null(params$ORDER) || !is.null(params$FAMILY) || !is.null(params$GENUS) || !is.null(params$SPECIES))
      "<li>Filtered results by pest taxonomy using fuzzy matching</li>" else "",
    "<li>Obtained the final determinations by:",
    "<ul>",
    "<li>Filtering out determinations flagged as not possible</li>",
    "<li>Selecting the determinations with the highest type ID</li>",
    "<li>Prioritizing determinations by highest expertise group ID</li>",
    "<li>Using the most recent determination when multiple options existed</li>",
    "</ul></li>",
    "</ol>",
    
    # Results summary
    "<h2>Results Summary</h2>",
    "<p>The search returned <strong>", nrow(df_diagnostic_results), "</strong> ultimate identifications matching the criteria.</p>",
    
    # Data access section
    "<h2>Accessing the Data</h2>",
    "<p>The complete interactive dataset can be accessed <a href='", paste0(base_filename, "_data.html"), "'>here</a>.</p>",
    "<p>The interactive data viewer provides options to copy, download as CSV, Excel, or PDF using the buttons at the top of the table.</p>",
    
    # Footer
    "<div class='timestamp'>Report generated by the ARM Pest Interception Analysis Tool</div>",
    "</body></html>"
  )
...
report_html <- generate_search_report()
writeLines(report_html, paste0(base_filename, "_report.html"))
...
cat(paste0("- Search report: ", base_filename, "_report.html\n"))
