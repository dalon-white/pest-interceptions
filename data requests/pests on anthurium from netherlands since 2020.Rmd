...
begin_date <- if(is.na(params$begin_date)) as.Date("01-01-1900", "%d-%m-%Y") else(as.Date(params$begin_date, "%d-%m-%Y"))

end_date <- if(params$end_date == "today") as.Date(Sys.Date(), "%d-%m-%Y") else(as.Date(params$end_date, "%d-%m-%Y"))
...
#previous version that has been replaced (in case needed later):
# arm_results = df_diagnostic_results |> 
#   filter(
#     INSPECTION_DATE >= begin_date,
#     INSPECTION_DATE <= end_date,
...
#     COMMODITY_DISPLAY_NAME %in% common_name | COMMODITY_TAXONOMIC_DISPLAY_NAME %in% taxonomic_name |
#       grepl(taxonomic_name, COMMODITY_TAXONOMIC_DISPLAY_NAME)
...
) |>  filter(DIAGNOSTIC_DETERMINATION_ID %in% (diag.determ.records |> pull(DIAGNOSTIC_DETERMINATION_ID)))
...
# Creates an interactive HTML table in RStudio viewer
